...
namespace ThreadWar
{
    ...
    {
        private static Thread badGuyThread;
        private static Thread moveThread;

        private static EventWaitHandle startEventWaitHandle;
        // Ожидает выполнение потока
        private static Mutex screenLock = new Mutex();
        // Ограничивает кол-во потоков
        private static Semaphore bulletSemaphore;

        private static List<Thread> badGuysTheads = new List<Thread>();
        private static List<Thread> bulletsThreads = new List<Thread>();
        ...
        private static int cHeight = 25;
        private static int cWidth = 80;
        private static char[,] cMatrixChars = new char[cHeight + 1, cWidth + 1];
